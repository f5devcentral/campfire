...
my $diffonly = 0;	# output differences only

sub usage {
	print STDERR <<USAGE_END;
USAGE: $0 [-hnsd] folded1 folded2 | flamegraph.pl > diff2.svg
	    -h       # help message
	    -n       # normalize sample counts
	    -s       # strip hex numbers (addresses)
	    -d		 # show diff only (hide equal counts)
...
foreach my $stack (keys %Folded) {
	...
	if ((not $diffonly) || ($diffonly && ($Folded{$stack}{1} != $Folded{$stack}{2})))	{
